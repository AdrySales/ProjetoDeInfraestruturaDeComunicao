...
class Handler:
  ...
  def conectar_usuario(self, nome, endereco):
    ...
    for i, line in enumerate(lines):
        ...
        if user_data[1] == nome:
            ...
            if user_data[2] == endereco:
                lines[i] ="\n" + ",".join(user_data) + "\n" if not last_line == i else ",".join(user_data)
            else:
    ...
    with open("usuarios.csv", "a") as file:
        file.write(f"{last_id + 1},{nome},{endereco},True\n")

    return user_data

  def checarReserva(self,room, day, time):
          with open('reservas.csv', 'r') as file:
              reader = csv.reader(file)
              next(reader)  # Pula a primeira linha
              for row in reader:
                if len(row) >= 4 and row[1] == room and row[2] == day and row[3] == time :
                      return False
          return True

  def fazerReserva(self,room,day,time,person_id):
        if self.checarReserva(room,day,time):
  ...
  def verificarDisponibilidade(self,sala, dia):
    ...
    horarios_padrao = ["06:00", "07:00", "08:00","09:00",  "10:00", "11:00", "12:00","13:00","14:00", "15:00", "16:00" "17:00"]
